...
function airAttribution(){const p=new URLSearchParams(location.search);return {source:'website',page:location.href,referrer:document.referrer,campaign:['utm_source','utm_medium','utm_campaign','utm_content'].map(k=>p.get(k)?k+'='+p.get(k):'').filter(Boolean).join('&')}}
async function submitAirRequest(payload){const response=await fetch('/api/air-submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...airAttribution(),...payload})});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'We could not complete the handoff. Please try again.');return data}
...
function openContact(withResults){
 ...
 body.innerHTML='<h2 id="contact-title">'+(withResults?'Take the full picture<br>with you.':'Tell us what’s<br>getting in the way.')+'</h2><p>'+(withResults?'Share your email to receive your six-dimension summary and a copy of your responses. You can also ask to talk it through.':'Tell us where technology could help your business. We’ll start with the work, the people, and what needs to get better.')+'</p>'+(withResults?'<div class="contact-context"><span class="eyebrow">YOUR PRACTICAL NEXT STEP</span><p>'+resultModel(answers).next+'</p></div>':'')+'<form id="contact-form"><label for="contact-name">Name</label><input id="contact-name" name="name" autocomplete="name" maxlength="100" required><label for="contact-email">Work email</label><input id="contact-email" name="email" type="email" autocomplete="email" maxlength="254" required><label for="contact-company">Company <span>(optional)</span></label><input id="contact-company" name="company" autocomplete="organization" maxlength="160"><label for="contact-context">What would you like to work through? <span>(optional)</span></label><textarea id="contact-context" name="context" rows="3" maxlength="1500"></textarea><input name="website" tabindex="-1" autocomplete="off" aria-hidden="true" class="air-honeypot">'+(withResults?'<label class="contact-consent"><input type="checkbox" required name="deliveryConsent"><span>Send my readiness picture to the email above and use my assessment answers to prepare it.</span></label><label class="contact-consent"><input type="checkbox" name="contactConsent"><span>I’d also like AI Resulting to contact me about my results.</span></label><p class="result-note">Receiving your picture does not subscribe you to marketing.</p><button class="button primary" type="submit">Email my full picture <span>↗</span></button>':'<label class="contact-consent"><input type="checkbox" required name="contactConsent"><span>I’d like AI Resulting to contact me about this request.</span></label><p class="result-note">Requesting a conversation does not subscribe you to marketing.</p><button class="button primary" type="submit">Send my request <span>↗</span></button>')+'<p class="contact-status" role="status" hidden></p></form>'+(withResults?'<button class="back" id="back-results">← Back to my picture</button>':'');
 body.querySelector('form').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget;if(!form.reportValidity())return;const data=new FormData(form),button=form.querySelector('[type="submit"]'),status=form.querySelector('.contact-status');button.disabled=true;status.hidden=false;status.textContent='Sending your request…';try{const m=resultModel(answers);const result=await submitAirRequest({type:withResults?'assessment':'contact',name:data.get('name'),email:data.get('email'),company:data.get('company'),context:data.get('context'),website:data.get('website'),deliveryConsent:data.get('deliveryConsent')==='on',contactConsent:data.get('contactConsent')==='on',assessmentSummary:withResults?[m.headline,m.strength,m.gap,'Next step: '+m.next].join('\n'):undefined,assessmentPriorities:withResults?intent.map(i=>INTENTS[i]).join(', '):undefined,assessmentResponses:withResults?assessmentText():undefined});form.reset();const booking=result.bookingUrl&&(data.get('contactConsent')==='on'||!withResults)?'<p><a class="button primary" href="'+escapeHtml(result.bookingUrl)+'">Book a 30-minute call <span>↗</span></a></p>':'';body.innerHTML='<h2 id="contact-title" tabindex="-1">That’s the handoff.</h2><p class="contact-status" role="status">'+(withResults?(result.deliveryStatus==='delivered'?'Your readiness picture is on its way.':'Your request is recorded and delivery is queued.'):'Your request has been sent to AI Resulting.')+'</p>'+booking+'<button class="back" id="contact-done">'+(withResults?'Return to my picture':'Back to exploring')+'</button>';body.querySelector('#contact-done').onclick=()=>{contact.close();if(withResults){renderResults();openDialog(assessment)}};body.querySelector('h2').focus()}catch(error){status.textContent=error.message;button.disabled=false}};
